...
:module
  ...
  rem call "%_mod_util%\exec" "%_hash% -u -t -r *" "known.txt" "%_mod_name%"
    call "%_mod_util%\exec" "%_hash% -u -t -r %%j*" "%_mod_output%\hashed.log" "%_mod_name%"
  ) 
  for /F %%j in (%_mod_tools%\recursive.txt) do (
    call "%_mod_util%\exec" "%_hash% -u -t %%j*" "%_mod_output%\hashed.log" "%_mod_name%"
  )
